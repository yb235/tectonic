...
% Also enforce new-page starts for top-level sections
...
% Align rating box and estimates block at the top
\begin{minipage}[t]{0.3\textwidth}
    \vspace{0pt}
    \ratingbox{Company Name (TICKER)}{Industry}{RATING}
    \vspace{0.2cm}
    \textbf{Risk:} HIGH \\
    \textbf{Target Price:} \$00.00
\end{minipage}%
\hfill
\begin{minipage}[t]{0.65\textwidth}
    \vspace{0pt}
    \estimatesbox{
        % Inside minipage, use \linewidth (not \textwidth)
        \begin{adjustbox}{max width=\linewidth}
        \begin{tabular}{lr}
            \textbf{Metric} & \textbf{Projection} \\
            \midrule
            2030E Revenue & \$0.0 Billion \\
            Peak Sales Potential & \$0.0 Billion \\
            Target Penetration (Region) & 0\% \\
            Addressable Patient Pool & $\sim$0--0 Million \\
        \end{tabular}
        \end{adjustbox}
    }
\end{minipage}
...
% --- TABLE OF CONTENTS ---
% Use the canonical TOC block (TOC body must not be boxed)
\newpage
...
\begin{axis}[
    msstyle,
    width=10cm,
    % Always brace symbolic labels to avoid pgfplots parsing errors
...
% Notes:
% - Inside multicols, size content with \columnwidth (never \textwidth).
% - Avoid Markdown syntax (e.g., **bold**) in .tex files.
