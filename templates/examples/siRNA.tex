...
\lhead{
    \raisebox{-5pt}[0pt][0pt]{% Lower James Bian to align top with TECH VISION
...
% Main content text is narrower than executive summary
\newenvironment{maincontent}{%
    \begin{adjustwidth}{3cm}{3cm}% Narrower margins for body text
    \let\oldsection\section%
    \renewcommand{\section}{\clearpage\oldsection}% Start each major section on a new page
}{%
...
\begin{table}[H]
\centering
\caption{The "Asian Paradox": Epidemiology Disparities}
\label{tab:epidemiology}
\tablefont
\rowcolors{2}{lightgrey}{white}
\begin{adjustbox}{max width=\columnwidth, center}
\begin{tabular}{p{0.35\columnwidth} p{0.25\columnwidth} p{0.35\columnwidth}}
\toprule
...
\begin{figure}[H]
\centering
\caption{The "Asian Paradox" creates a localized blockbuster opportunity}
\label{fig:asian_paradox}
\begin{tikzpicture}
\begin{axis}[
    msstyle,
    ...
    bar width=25pt,
    nodes near coords,
    nodes near coords style={font=\footnotesize, color=black, yshift=5pt},
]
...
%--- MAIN BODY CONTENT (ONE-COLUMN LAYOUT) ---
\begin{maincontent}

\section{Market Opportunity \& Revenue Framework}
...
\begin{enumerate}
    ...
    \item \textbf{The "Surgical Alternative" Model:} Price point of \$2,000--\$5,000 per course. This is the "sweet spot." The cost of LEEP/Conization ranges from \textbf{\$1,500 to \$3,000} in direct procedural costs, but the \textit{total cost of care} rises significantly when factoring in complications.
\end{enumerate}
...
\begin{table}[H]
\centering
\caption{Valuation Model: Scenario Sensitivity Analysis (2030E)}
\label{tab:valuation_models}
\tablefont
\rowcolors{2}{lightgrey}{white}
\begin{tabular}{p{3cm} p{2.5cm} p{3.5cm} p{4cm}}
\toprule
\rowcolor{tableheader} \textbf{Valuation Scenario} & \textbf{Price Point} & \textbf{Est. Global Peak Sales} & \textbf{Strategic Viability} \\ 
\midrule
\textbf{"Orphan" Model} & \$300,000+ & $<$ \$500M & \textbf{Low.} Volume too high for orphan designation; payers mandate LEEP. \\ 
\textbf{"Chronic" Model} & \$3,250 - \$6,500 & \$5B - \$10B & \textbf{Moderate.} Recurring revenue attractive, but "cure" is preferred endpoint. \\ 
...
\textbf{"Premium Vaccine"} & \$500 - \$1,000 & \$500M - \$1B & \textbf{Low.} High COGS for siRNA/LNPs erodes margins at this price floor. \\ 
\bottomrule
\end{tabular}
\par\vspace{0.1cm}
{\tiny Source: James Bian Research}
...
\subsection{The China Patient Funnel: Deriving the Serviceable Obtainable Market (SOM)}
...
\begin{figure}[H]
\centering
\caption{China HPV-58 Patient Funnel (Estimated Addressable Market)}
\label{fig:patient_funnel}
\begin{tikzpicture}
\begin{axis}[
    msstyle,
    xbar, 
    ...
    nodes near coords, 
    nodes near coords style={font=\footnotesize, color=black, xshift=8pt}, 
    xmin=0, xmax=550,
...
\textbf{Funnel Implication:} Even with conservative filtering for diagnostic gaps and treatment penetration, the SOM in China alone is \textbf{approximately 600,000 to 1.5 million courses annually}. At a \$3,000 price point, this supports a China-only revenue baseline of \textbf{\$1.8 billion to \$4.5 billion}, validating the "East Asia First" strategy.

\subsection{The "Catch-Up" Moat: Why Vaccines Don't Kill the Market}
...
\subsection{Geographic Segmentation \& Sensitivity Analysis}
...
\begin{table}[H]
\centering
\caption{Pricing Elasticity Sensitivity Analysis (Annual Revenue Potential)}
\label{tab:sensitivity}
\tablefont
\rowcolors{2}{lightgrey}{white}
\begin{tabular}{l c c c c}
\toprule
\rowcolor{tableheader} & \multicolumn{4}{c}{\textbf{Market Penetration Rate (of Diagnosed Pool)}} \\ 
\rowcolor{tableheader} \textbf{Price per Course} & \textbf{5\% (Niche)} & \textbf{10\% (Moderate)} & \textbf{20\% (High)} & \textbf{30\% (Dominant)} \\ 
\midrule
\textbf{\$1,500 (Low)} & \$337M & \$675M & \$1.35B & \$2.02B \\ 
\textbf{\$3,000 (Base)} & \$675M & \$1.35B & \$2.70B & \$4.05B \\ 
...
\subsection{The Diagnostic Catalyst: No Test, No Treatment}
...
\section{Scientific Rationale: Mechanism, Targets, \& Delivery}
...
\subsection{The Specificity Trap: Why HPV-16 Data Does Not Apply}
...
\begin{figure}[H]
\centering
\caption{Comparison of Invasive Cervical Cancer Risk between HPV-58 Wild Type and high-risk T20I/G63S variants}
\label{fig:variant_threat}
\begin{tikzpicture}
    \begin{axis}[
        msstyle,
        width=0.85\textwidth,
...
\end{tikzpicture}
\par\vspace{0.1cm}
{\tiny Source: Internal Analysis of Epidemiological Data}
\end{figure}
...
\subsection{The Delivery Challenge: Crossing the Mucosal Barrier}
...
\begin{table}[H]
\centering
\caption{Comparative Analysis of Delivery Platforms for Cervical siRNA}
\label{tab:delivery}
\tablefont
\rowcolors{2}{lightgrey}{white}
\begin{tabular}{p{2.5cm} p{4cm} p{4cm} p{3cm}}
\toprule
\rowcolor{tableheader} \textbf{Vehicle Type} & \textbf{Mechanism of Penetration} & \textbf{Advantages} & \textbf{Current Status} \\ 
\midrule
...
\textbf{Transdermal Peptides (e.g., PKU12)} & Cell-penetrating peptides (CPPs) facilitate transport across dermis/mucosa. & Non-viral; potentially better tissue penetration depth. & Early Preclinical (Mouse Models) \\ 
...
\textbf{GalNAc Conjugates} & ASGPR-ligand binding. & \textbf{Ineffective} for cervix (ASGPR is liver-specific). & \textbf{Not Applicable} \\ 
\bottomrule
\end{tabular}
\par\vspace{0.1cm}
{\tiny Source: James Bian Research}
...
\subsection{Differentiation vs. Gene Editing (CRISPR)}
...
\subsection{Summary of Preclinical Requirements}

To unlock the \$500M+ opportunity, development programs must move beyond standard subcutaneous xenograft models. The critical path to Investigational New Drug (IND) approval requires \textbf{orthotopic mouse models} (direct cervical injection/application) to prove that the selected delivery vehicle can physically traverse the mucus barrier and silence genes in the basal layer \textit{in vivo}. Programs relying solely on \textit{in vitro} data or flank tumors are likely to fail in Phase I due to the specific challenges of the cervicovaginal microenvironment.

\section{Competitive Landscape: Vaccines, Surgery, \& Emerging Tech}
...
\begin{table}[H]
\centering
\caption{Comparative Matrix: siRNA vs. Standard of Care (LEEP)}
\label{tab:soc_comparison}
\tablefont
\rowcolors{2}{lightgrey}{white}
\begin{tabular}{p{3.5cm} p{4cm} p{4cm}}
\toprule
\rowcolor{tableheader} \textbf{Feature} & \textbf{LEEP / Conization (SOC)} & \textbf{HPV-58 siRNA (Target Profile)} \\ 
\midrule
\textbf{Mechanism} & Physical removal of lesion. & Molecular silencing of E6/E7 oncogenes. \\ 
\textbf{Recurrence Rate} & \textbf{20--55\%} (Removes lesion, not viral reservoir). & Potential for lower recurrence by clearing viral factory. \\ 
\textbf{Morbidity Risks} & Preterm birth, cervical stenosis, bleeding. & Non-invasive; preserves cervical integrity. \\ 
\textbf{Target Patient} & High-Grade Lesions (CIN2/3). & Persistent Infection + CIN1/2/3. \\ 
...
\begin{figure}[H]
\centering
\caption{Projected persistence of the "Catch-Up" patient pool in China over two decades}
\label{fig:patient_pool}
\begin{tikzpicture}
\begin{axis}[
    msstyle,
...
By solving the mucosal delivery challenge, a specialized biotech can exploit this \textbf{"Asian Paradox"}---leveraging Western RNAi technology to solve an Eastern epidemiological crisis---securing a natural monopoly in a market of millions of women who are currently managing their infection with "watchful waiting" or invasive surgery.
...
\end{adjustbox}
\par\vspace{0.1cm}
{\tiny Source: James Bian Research, Analysis of Clinical Data}
\end{table}
...
\begin{figure}[H]
\centering
\caption{Projected Timeline to Market Entry (Years from Inception)}
\label{fig:timeline_to_market}
\begin{tikzpicture}
\begin{axis}[
    xbar,
    xmin=0, xmax=9,
...
\end{tikzpicture}
\par\vspace{0.1cm}
{\tiny Source: James Bian Research, Industry Benchmarks}
\end{figure}
...
\end{adjustbox}
\par\vspace{0.1cm}
{\tiny Source: James Bian Research, Industry Data}
\end{table}
...
\end{tikzpicture}
\vspace{0.1cm}
{\tiny Source: Market projections indicate the total oligonucleotide synthesis market reaching \$22.5B by 2028 (CAGR 17.6\%), while the specific siRNA therapeutics market is projected to reach $\sim$\$6.89B by 2033.}
\end{figure}
...
\end{tikzpicture}
\par\vspace{0.1cm}
{\tiny Source: Comparative economic analysis derived from standard-of-care procedure costs. While siRNA targets the \$2,000-\$5,000 range, it must compete with low-cost Cryotherapy in LMICs and justify its premium against base LEEP costs by offsetting high complication costs (e.g., preterm birth care)}
\end{figure}
...
\toprule
\rowcolor{tableheader} \textbf{Component} & \textbf{Launch (2028E)} & \textbf{Mature (2032E)} & \textbf{Driver of Change} \\
\midrule
\textbf{List Price (WAC)} & \$2,500 & \$2,200 & Volume-based pricing pressure from centralized procurement (China VBP). \\
\rowcolor{lightgrey} \textbf{Rebates/Discounts} & (15\%) & (25\%) & Increased payer negotiation leverage over time. \\
\textbf{Net Price} & \textbf{\$2,125} & \textbf{\$1,650} & \\
\midrule
API Synthesis Cost & \$250 & \$100 & Scale-up efficiencies; move to enzymatic synthesis. \\
\rowcolor{lightgrey} Formulation (LNP) & \$200 & \$120 & Microfluidic optimization; lower lipid raw material costs. \\
Fill/Finish \& Packaging & \$50 & \$30 & Standard efficiencies. \\
\textbf{Total COGS} & \textbf{\$500} & \textbf{\$250} & \\
\midrule
\rowcolor{tableheader} \textbf{Gross Profit} & \textbf{\$1,625} & \textbf{\$1,400} & \\
...
\rowcolor{tableheader} & \textbf{\$2,000} & \$625 & \textbf{\$1,250} & \textbf{\$1,875} & \textbf{\$2,500} & \$3,750 \\
...
\begin{axis}[
    ...
    nodes near coords={}, 
    every node near coord/.append style={font=\footnotesize, color=white}
]
    ...
    % US / EU (Low Burden)
    \addplot[fill=lightgrey] coordinates {
...
\toprule
\rowcolor{tableheader} \textbf{Metric (\$ Millions)} & \textbf{2028E (Launch)} & \textbf{2029E} & \textbf{2030E} & \textbf{2031E} & \textbf{2032E} \\
\midrule
...
\end{adjustbox}
\par\vspace{0.1cm}
{\tiny Note: 2028-2029 losses reflect heavy SG\&A launch costs and final Phase III wrap-up. Profitability inflects sharply in 2030 due to high gross margins and low marginal production costs}
\end{table}
...
\toprule
\rowcolor{tableheader} \textbf{Macro Indicator} & \textbf{Current Value} & \textbf{Trend / Projection} & \textbf{Impact on HPV-58 Strategy} \\
\midrule
Global Oligo Synthesis Market & \$7.3B (2021) & \$22.5B (2028E) & Intense competition for GMP capacity may drive up initial COGS. \\
\rowcolor{lightgrey} Global siRNA Market Growth & \$2.55B (2024) & CAGR 14.4\% (to 2033) & Strong investor appetite for RNAi platforms supports capital raising. \\
...
\rowcolor{lightgrey} Manufacturing Capacity & Tight & H-Occupancy Constraints & Limited "High Hazard" facilities for solvent-heavy chemical synthesis creates bottlenecks. \\
...
\begin{figure}[H]
\centering
\caption{Global Oligonucleotide Synthesis Market Trajectory (2021-2028E)}
\label{fig:oligo_trajectory}
\begin{tikzpicture}
\begin{axis}[
    ybar,
    msstyle,
...
\end{tikzpicture}
\par\vspace{0.1cm}
{\tiny Explosive Growth in Oligo Synthesis Capacity. The projected tripling of the market by 2028 indicates a maturing supply chain that will eventually support mass-market volumes for indications like HPV-58. Source: Industry Data}
\end{figure}
