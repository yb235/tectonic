...
\begin{itemize}[leftmargin=*]
    \item \textbf{Intel 18A Timeline:} Risk production commenced Q2 2025; rumors suggest Apple low-end M-series adoption by H2 2027.
    \item \textbf{Yield Considerations:} External reports indicate 18A test wafer yields near $\sim$10\% (mid-2025), while Intel management claims 60--65\% for internal products; Apple's commercial viability threshold is $>$70\%.
    \item \textbf{Foundry Financials:} IFS operating losses reached \$13.0B in 2024; break-even target remains 2027.
\end{itemize}
}
...
While the strategic rationale is sound, execution risk remains the dominant variable. With external reports indicating 18A test wafer yields near $\sim$10\% in mid-2025 (versus Intel management's claims of 60--65\% for internal products), Intel has a substantial gap to close to meet Apple's $>$70\% commercial viability standard. We classify this potential partnership as \textbf{probationary}: Apple has engaged, but full production remains contingent on Intel proving it can deliver yield, volume, and performance stability by Q1 2026.
...
\end{axis}
\end{tikzpicture}
\caption{Projected path to break-even for Intel Foundry, contingent on successful 18A ramp and external customer (Apple) volume in 2027/2028.}
\label{fig:foundry_breakeven_projection}
\end{figure}
...
\begin{table}
    ...
    \end{adjustbox}
    \par\vspace{0.1cm}
    {\footnotesize \textit{Source: James Bian Research Estimates. Note: Bear case assumes IFS failure and potential restructuring; Bull case assumes successful Apple 18A ramp and GM parity with TSMC.}}
...
\end{axis}
\end{tikzpicture}
\caption{Implied Enterprise Value to Sales (EV/Sales) multiples for Intel Foundry Services under different execution scenarios compared to TSMC's current trading multiple. The "Bull Case" assumes successful high-volume manufacturing (HVM) for Apple M-series chips.}
\label{fig:multiples}
\end{figure}
...
\textit{\small Projected sensitivity of Intel Foundry Services (IFS) Operating Margins in 2027 based on 18A process node yield rates. Current rumors place yields near the "Yield Failure" zone, requiring a massive ramp to achieve the "Target" scenario by 2027.}
\end{table}
